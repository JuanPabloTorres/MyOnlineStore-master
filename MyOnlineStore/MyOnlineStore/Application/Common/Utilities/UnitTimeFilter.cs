    ...
    static class UnitTimeFilter
    {
        ...
        public static List<Record> GetRecordByMonths(DateTime from, DateTime to, IEnumerable<Order> orders, bool status)
        {
            ...
            for (var day = from.Date; day.Date <= to.Date; day = day.AddDays(1))
            {
                ...
                record.RecordValue = orderbyweek.Where(c => c.OrderDate == day && c.OrderStatus == OrderStatus.Pending).Sum(i => i.OrderItems.Sum(s => s.ProductItem.Price));
        ...
        public static List<Record> GetRecordbyWeek(DateTime from, DateTime to, IEnumerable<Order> orders)
        {
            ...
            for (var day = from.Date; day.Date <= to.Date; day = day.AddDays(1))
            {
                ...
                record.RecordValue = orderbyweek.Where(c => c.OrderDate == day).Sum(i => i.OrderItems.Sum(s => s.ProductItem.Price));
